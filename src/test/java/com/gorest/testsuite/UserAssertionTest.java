...
public class UserAssertionTest extends TestBase {
    ...
    @Test
    public void test002() {
        response.body("find{it.id==5228}.name",equalTo("Dipankar Khanna"));
    }

    //3. Check the single ‘Name’ in the Array list (Subhashini Talwar)
    @Test
    public void test003() {
        response.body("[8].name",equalTo("Gopee Nayar"));
    }

    //4. Check the multiple ‘Names’ in the ArrayList (Mrs. Menaka Bharadwaj, Msgr. Bodhan Guha, Karthik Dubashi IV)
    @Test
    public void test004() {
        response.body("name",hasItems("Bhilangana Shah","Ganak Jain","Pres. Bhaaswar Adiga"));
    }

    //5. Verify the email of userid = 5471 is equal “adiga_aanjaneya_rep@jast.org”
    @Test
    public void test005() {
        response.body("find{it.id==5221}.email",equalTo("ii_anamika_dubashi@hilpert-crona.name"));
    }
    ...
    @Test
    public void test007() {
        response.body("find{it.name=='Chaitan Chaturvedi'}.gender",equalTo("male"));
    }
}
